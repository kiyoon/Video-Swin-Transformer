...
train_pipeline = [
    ...
    dict(type='SampleFrames', clip_len=96, frame_interval=1, num_clips=1, frame_uniform=True),
    ...
    dict(type='ToTensor', keys=['imgs', 'label'], GreyST=True)
]
val_pipeline = [
    dict(type='DecordInit'),
    dict(
    ...
    dict(type='FormatShape', input_format='NCTHW'),
    dict(type='Collect', keys=['imgs', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['imgs'], GreyST=True)
]
test_pipeline = [
    dict(type='DecordInit'),
    dict(
...
work_dir = './work_dirs/sthv2_swin_base_patch244_window1677_GreyST.py'
